...
df_test = pd.read_csv(kaggle_folder+"alternate_dimension\\data\\test.csv")
...
class TestPreprocessing:
    ...
    def test_apply_imputation(self):
        ...
        preprocessing = AlternateDimPreprocessing(df=df1, target_col=target_col, test=False)
        df1 = preprocessing.apply_imputation()
        assert not df1.isnull().values.any()

        df2 = df_test.copy()
        assert df2.isnull().values.any()
        preprocessing = AlternateDimPreprocessing(df=df2, target_col=target_col, test=True)
